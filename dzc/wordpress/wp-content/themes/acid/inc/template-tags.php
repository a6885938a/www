...
?>
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( 'Post navigation', 'puremellow' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'puremellow' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'puremellow' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'puremellow' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'puremellow' ) ); ?></div>
		<?php endif; ?>
...
?>
	<li class="post pingback">
		<p><?php _e( 'Pingback:', 'puremellow' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(Edit)', 'puremellow' ), ' ' ); ?></p>
	<?php
		break;
	default :
			...
			<div class="comment__container cf">
				...
				<footer class="comment-author g one-sixth lap-one-fifth palm-one-whole">
					...
					<div class="comment-meta commentmetadata">						
						<?php edit_comment_link( __( '(Edit)', 'puremellow' ), ' ' );?>
				...
				<div class="comment-content g five-sixths lap-four-fifths palm-one-whole">
					...
					<?php if ( $comment->comment_approved == '0' ) : ?>
						<em><?php _e( 'Your comment is awaiting moderation.', 'puremellow' ); ?></em>
						<br />
					<?php endif; ?>
