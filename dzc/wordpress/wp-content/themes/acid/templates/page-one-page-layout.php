...
$menu_items = Pure_One_Page::page_get_menu_items("pure_one_page");

$query = new WP_Query(array(
            'post_type' => 'page',
            'post__in' => $menu_items,
            'posts_per_page' => intval( Pure::get_theme_mod( "one_page_PPP", 4 ) ), 
            'orderby' => 'post__in',
            'paged' => $paged,
			));
	...
	<div id="scrollbar">
		<div id="primary" class="viewport" data-horizontal-scroll="on">
			...
			while ( $query->have_posts() ) : $query->the_post(); 
				...
				$template_name = Pure::get_from_meta($meta, "_wp_page_template", null);
				...
				$page_title_enabled = Pure::get_from_meta($meta, "acid_page_title_enabled", true);
				
				// Colors
				$page_color = sanitize_hex_color( Pure::get_from_meta( $meta, "acid_page_color" ) );
				$page_font_color = sanitize_hex_color( Pure::get_from_meta( $meta, "acid_page_font_color" ) );
				$page_title_color = sanitize_hex_color( Pure::get_from_meta( $meta, "acid_page_title_color" ) );
				$page_title_font_color = sanitize_hex_color( Pure::get_from_meta( $meta, "acid_page_title_font_color" ) );
				
				// Page Width
				$page_width = Pure::get_from_meta($meta, "acid_page_width", "960");
